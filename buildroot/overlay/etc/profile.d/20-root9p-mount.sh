...
# Shell startup fallback: start a background task that mounts host 9p on /root.
# Keep shell startup fast even if virtio/9p is late or missing.
if ! grep -qs ' /root 9p ' /proc/mounts; then
    _root9p_state_dir="/run"
    [ -d "$_root9p_state_dir" ] || _root9p_state_dir="/tmp"
    _root9p_marker="$_root9p_state_dir/root9p-mount-bg.started"

    if [ ! -e "$_root9p_marker" ]; then
        : > "$_root9p_marker" 2>/dev/null || true
        (
            ...
            _root9p_i=0
            while [ "$_root9p_i" -lt 120 ] && ! grep -qs ' /root 9p ' /proc/mounts; do
